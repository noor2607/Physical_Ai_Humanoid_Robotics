#!/usr/bin/env python3
"""
Script to run the textbook content ingestion from sitemap using Cohere embeddings as primary,
with fallback to Gemini if Cohere is not available.
"""
import asyncio
import sys
import os
...
class EmbeddingService:
    """
    Service for generating embeddings with fallback between Cohere and Gemini
    """
    ...
    def _initialize_services(self):
        """Initialize both Cohere and Gemini services if possible"""
        # Try to initialize Cohere
        try:
            from services.cohere_embedding import cohere_embedding_service
            self.cohere_service = cohere_embedding_service
            self.primary_service = self.cohere_service
            logger.info("Cohere embedding service initialized successfully")
        except Exception as e:
            logger.warning(f"Cohere service initialization failed: {str(e)}")

        # Try to initialize Gemini
        try:
            import google.generativeai as genai
            from services.embedding import embedding_service
            self.gemini_service = embedding_service
        ...
        if self.primary_service is None:
            raise Exception("No embedding service available (neither Cohere nor Gemini)")

        logger.info(f"Using {type(self.primary_service).__name__} as primary embedding service")

    async def generate_embedding(self, text: str) -> List[float]:
        """Generate embedding for text using the primary service"""
        try:
            return await self.primary_service.generate_embedding(text)
        except Exception as e:
            logger.error(f"Primary embedding service failed: {str(e)}")
            # Try fallback service
            fallback_service = self.gemini_service if self.primary_service == self.cohere_service else self.cohere_service
            if fallback_service:
                logger.info("Trying fallback embedding service...")
                try:
                    return await fallback_service.generate_embedding(text)
                except Exception as fallback_error:
                    logger.error(f"Fallback embedding service also failed: {str(fallback_error)}")
                    raise
            else:
                raise

    async def generate_query_embedding(self, query: str) -> List[float]:
        """Generate query embedding using the primary service"""
        try:
            return await self.primary_service.generate_query_embedding(query)
        except Exception as e:
            logger.error(f"Primary query embedding service failed: {str(e)}")
            # Try fallback service
            fallback_service = self.gemini_service if self.primary_service == self.cohere_service else self.cohere_service
            if fallback_service:
                logger.info("Trying fallback query embedding service...")
                try:
                    return await fallback_service.generate_query_embedding(query)
                except Exception as fallback_error:
                    logger.error(f"Fallback query embedding service also failed: {str(fallback_error)}")
                    raise
            else:
                raise

    def validate_embedding(self, embedding: List[float]) -> bool:
        """Validate embedding using the primary service"""
        return self.primary_service.validate_embedding(embedding)
                ...
                if not collection_exists:
                    ...
                    self.client.create_collection(
                        ...
                        vectors_config=models.VectorParams(
                            size=settings.embedding_dimensions,  # Updated to match Cohere: 1024
...
async def main():
    ...
    """
    logger.info("Starting textbook content ingestion from sitemap...")
