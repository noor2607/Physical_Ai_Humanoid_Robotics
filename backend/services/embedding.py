...
class EmbeddingService:
    """
    Service for generating embeddings using Google's Gen AI API
    """

    def __init__(self):
        # Store the API key for later use with embed_content
        self.api_key = settings.gemini_api_key

        # Get the embedding model name
        self.model_name = settings.embedding_model
        # We'll use the genai.embed_content function directly, no need to store it as an instance variable
    ...
    async def generate_embedding(self, text: str) -> List[float]:
        ...
        try:
            ...
            if hasattr(response, 'embedding'):
                ...
                embedding = response.values if hasattr(response, 'values') else response
    ...
    async def generate_query_embedding(self, query: str) -> List[float]:
        """
        Generate embedding for a query specifically for retrieval tasks
